 ...
 * under the License.
 */
package org.cs.infrastructure.security.service;
...
@Service
public class CS_SpringSecurityPlatformSecurityContext implements CS_PlatformSecurityContext {
    ...
    protected static final List<CS_CommandWrapper> EXEMPT_FROM_PASSWORD_RESET_CHECK = new ArrayList<CS_CommandWrapper>() {

        {
            add(new CS_CommandWrapperBuilder().updateUser(null).build());
        }
    };

    @Autowired
    CS_SpringSecurityPlatformSecurityContext(final ConfigurationDomainService configurationDomainService) {
    ...
    @Override
    public AppUser authenticatedUser(CS_CommandWrapper commandWrapper) {
    ...
    private boolean shouldCheckForPasswordForceReset(CS_CommandWrapper commandWrapper) {
        for (CS_CommandWrapper commandItem : EXEMPT_FROM_PASSWORD_RESET_CHECK) {
