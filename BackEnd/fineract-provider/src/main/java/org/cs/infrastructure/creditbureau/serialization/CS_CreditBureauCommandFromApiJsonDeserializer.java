 ...
 * under the License.
 */
package org.cs.infrastructure.creditbureau.serialization;
...
@Component
public class CS_CreditBureauCommandFromApiJsonDeserializer {

	private final Set<String> supportedParameters = new HashSet<>(Arrays.asList("alias", "is_active","creditBureauId", "name", "country", "product", "implementationKey"));

	private final FromJsonHelper fromApiJsonHelper;

	@Autowired
	public CS_CreditBureauCommandFromApiJsonDeserializer(final FromJsonHelper fromApiJsonHelper) {
	...
	public void validateForCreate(final String json) {
		...
		final String product = this.fromApiJsonHelper.extractStringNamed("product", element);
		baseDataValidator.reset().parameter("product").value(product).notNull().notBlank().notExceedingLengthOf(100);

		final String country = this.fromApiJsonHelper.extractStringNamed("country", element);
		baseDataValidator.reset().parameter("country").value(country).notNull().notBlank().notExceedingLengthOf(100);

		final String implementationKey = this.fromApiJsonHelper.extractStringNamed("implementationKey", element);
		baseDataValidator.reset().parameter("implementationKey").value(implementationKey).notNull().notBlank().notExceedingLengthOf(100);
