...
public class CS_CommandSource extends AbstractPersistableCustom<Long> {
    ...
    public static CS_CommandSource fullEntryFrom(final CS_CommandWrapper wrapper, final CS_JsonCommand command, final AppUser maker) {
        return new CS_CommandSource(wrapper.actionName(), wrapper.entityName(), wrapper.getHref(), command.entityId(), command.subentityId(),
                command.json(), maker, DateTime.now());
    }

    protected CS_CommandSource() {
        //
    }

    private CS_CommandSource(final String actionName, final String entityName, final String href, final Long resourceId,
