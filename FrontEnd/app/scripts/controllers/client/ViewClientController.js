        ...
        ViewClientController: function (scope, routeParams, route, location, resourceFactory, http, $uibModal, API_VERSION, $rootScope, Upload, creditBureauServices) {
           ...
           // scope.families=[];
            ...
            resourceFactory.clientAccountResource.get({clientId: routeParams.id}, function (data) {
                ...
                scope.revolvingLoan = scope.clientAccounts.loanAccounts.filter(function(currentLoan){
                    return currentLoan.productId == 11;
                });
                scope.clientAccounts.loanAccounts = scope.clientAccounts.loanAccounts.filter(function(currentLoan){
                    return currentLoan.productId != 11;
                });
            ...
            scope.setRevolving = function () {
                if (scope.openRevolving) {
                    scope.openRevolving = false
                } else {
                    scope.openRevolving = true;
                }
            };
            ...
            scope.changeCreditLeft = function(creditLimit){
                var sumOfLoan = 0;
                scope.revolvingLoan.forEach(function(data){
                    sumOfLoan = !data.originalLoan ? 0 + + sumOfLoan : data.originalLoan + + sumOfLoan;
                });
                scope.creditLeft = creditLimit - sumOfLoan;
            };
